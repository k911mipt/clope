...
export interface IRepository {
    ...
    GetDicSize(): number;
    ReadUntilEnd(handleTransaction: (tr: Transaction) => void): Promise<void>;
...
export class Repository<TIn> implements IRepository {
    ...
    private elementMaps: Array<Map<any, number>>;
    ...
    public GetDicSize(): number {
        let size = 0;
        this.elementMaps.forEach((map) => {
            size += map.size;
        });
        console.log("Размер словаря: ", size);
        return size;
    }

    public CreateTransaction(elements: any[]): Transaction {
        const transaction = new Transaction(elements.length);
        const transactionElement = new TransactionElement("", 0);
        for (let index = 0; index < elements.length; index++) {
            if (this.IsColumnSkipped(index)) { continue; }
            const element = elements[index];
            if (this.nullElements.has(element)) { continue; }
            transactionElement.value = element;
            transactionElement.number = index;
    ...
    public async ReadUntilEnd(HandleTransaction: (tr: Transaction) => void): Promise<void> {
        await this.dataSource.Connect();
        await this.dataSource.ReadNext((row: TIn) => {
            HandleTransaction(this.CreateTransaction(this.rowConverter.Convert(row)));
        });
    ...
    public GetClassesIDs(): ITransactionElement[] {
        ...
        // FIXME: сделать перебор по массиву мапов
    ...
    private IsColumnSkipped(index: number) {
